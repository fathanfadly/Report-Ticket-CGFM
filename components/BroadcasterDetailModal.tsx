...
interface BroadcasterDetailModalProps {
    isOpen: boolean;
    onClose: () => void;
    onEdit: (broadcaster: any) => void;
    broadcaster: any | null;
}

const BroadcasterDetailModal = ({ isOpen, onClose, onEdit, broadcaster }: BroadcasterDetailModalProps) => {
    if (!isOpen || !broadcaster) return null;
                ...
                <header className="flex h-20 items-center justify-between border-b border-gray-100 bg-white px-8 shrink-0">
                    <div className="flex items-center gap-4">
                        <div className="h-12 w-12 rounded-2xl bg-indigo-600 flex items-center justify-center text-white font-black text-xl shadow-lg shadow-indigo-100 border border-white/20">
                            {broadcaster.nama?.charAt(0) || 'B'}
                        </div>
                        <div>
                            <h2 className="text-xl font-black text-gray-900 tracking-tighter">{broadcaster.nama}</h2>
                            <div className="flex items-center gap-2 text-xs text-gray-400 font-bold uppercase tracking-wide">
                                <span>ID #{broadcaster.id}</span>
                                <span className="h-1 w-1 rounded-full bg-gray-300"></span>
                                <span className="text-indigo-600">{broadcaster.tipe_pelapor || 'Broadcaster'}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                <DetailItem icon={Phone} label="No HP" value={broadcaster.no_hp} colorClass="text-indigo-400" />
                                <DetailItem icon={MapPin} label="Alamat" value={broadcaster.alamat} colorClass="text-red-400" />
                                <div className="grid grid-cols-2 gap-3">
                                    <DetailItem icon={Calendar} label="Usia" value={broadcaster.usia ? `${broadcaster.usia} Tahun` : null} colorClass="text-orange-400" />
                                    <DetailItem icon={User} label="Jenis Kelamin" value={broadcaster.jenis_kelamin} colorClass="text-blue-400" />
                                </div>
                                <DetailItem icon={GraduationCap} label="Pendidikan" value={broadcaster.pendidikan} colorClass="text-purple-400" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                <DetailItem icon={Briefcase} label="Pekerjaan" value={broadcaster.pekerjaan} colorClass="text-emerald-400" />
                                <DetailItem icon={User} label="Jabatan" value={broadcaster.jabatan} colorClass="text-teal-400" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                <DetailItem icon={Heart} label="Hobi" value={broadcaster.hobi} colorClass="text-pink-400" />
                                <DetailItem icon={Music} label="Pilihan Jenis Lagu" value={broadcaster.pilihan_jenis_lagu} colorClass="text-violet-400" />
                                <div className="grid grid-cols-2 gap-3">
                                    <DetailItem icon={Car} label="Alat Transportasi" value={broadcaster.alat_transportasi} colorClass="text-slate-400" />
                                    <DetailItem icon={Smartphone} label="Range Harga Gadget" value={broadcaster.range_harga_gadget} colorClass="text-indigo-400" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-3">
                                <DetailItem icon={Radio} label="Radio Sering Diputar" value={broadcaster.radio_sering_diputar} colorClass="text-amber-400" />
                                <DetailItem icon={Radio} label="Acara Radio Favorit" value={broadcaster.acara_radio_favorit} colorClass="text-orange-400" />
                                <DetailItem icon={Tv} label="TV Sering Ditonton" value={broadcaster.tv_sering_ditonton} colorClass="text-rose-400" />
                                <DetailItem icon={Tv} label="Acara TV Favorit" value={broadcaster.acara_tv_favorit} colorClass="text-red-400" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-4 md:col-span-2">
                            ...
                            <DetailItem icon={MapPin} label="Objek Wisata Favorit" value={broadcaster.objek_wisata_favorit} colorClass="text-blue-400" />
                        </div>
                    </div>
                </div>
...
export default BroadcasterDetailModal;
